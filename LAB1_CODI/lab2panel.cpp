...
Lab2Panel::Lab2Panel(QWidget *parent) : QWidget(parent)
{
// Устанавливаем путь к папке с результатами (ДОЛЖЕН СОВПАДАТЬ с OctaveRunner)
#ifdef Q_OS_WIN
    m_resultsPath = QDir::tempPath() + "/bch_lab_results/";
#else
    m_resultsPath = QDir::home().filePath("bch_lab_results/");
#endif

    // Создаем папку если её нет (на всякий случай)
    QDir().mkpath(m_resultsPath);

    qDebug() << "Lab2Panel: Results path set to:" << m_resultsPath;
    ...
    connect(three, &QPushButton::clicked, this, [=](){
        ...
        if(input.isEmpty() || encoded.isEmpty() || decoded.isEmpty()) {
            emit logMessage("❌ Ошибка: Не удалось загрузить один из файлов данных");
            emit logMessage("   Путь к результатам: " + m_resultsPath);
            return;
        }
        ...
        QCPTextElement *titleInput = new QCPTextElement(plotInput, "Входное слово", QFont("sans", 10, QFont::Bold));
        plotInput->plotLayout()->insertRow(0);
        plotInput->plotLayout()->addElement(0, 0, titleInput);
        ...
        plotInput->graph(0)->setData(x_input, input);
        plotInput->graph(0)->setLineStyle(QCPGraph::lsNone);
        plotInput->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 4));
        plotInput->graph(0)->setPen(QPen(Qt::blue));
        ...
        mainLayout->addWidget(plotInput, 1);
        mainLayout->addWidget(plotEncoded, 1);
        mainLayout->addWidget(plotDecoded, 1);
...
// ===== Чтение CSV из правильной папки =====
QVector<double> Lab2Panel::readCsv(const QString &filePath)
{
    QVector<double> data;
    QString fullPath = m_resultsPath + filePath + ".csv";
    QFile file(fullPath);

    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "Cannot open file:" << fullPath;
        emit logMessage("❌ Не удалось открыть файл: " + fullPath);
        return data;
    }

    emit logMessage("✅ Загружен файл: " + fullPath);
    ...
    qDebug() << "Loaded" << data.size() << "elements from" << fullPath;
    return data;
}
...
void Lab2Panel::plotCsv(const QString &fileName)
{
    QVector<double> data = readCsv(fileName);
    if (data.isEmpty()) {
        emit logMessage("❌ Нет данных для отображения: " + fileName);
        return;
    }
